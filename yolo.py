...
from utils.utils import compose#这是作者自己写的库

from nets.darknet import DarknetConv2D, DarknetConv2D_BN_Leaky, darknet_body
from nets.yolo_training import yolo_loss#这是作者自己写的库
...
def yolo_body(input_shape, anchors_mask, num_classes):
    ...
    x   = Concatenate()([x, C3])#concat:[1,2,3,]和[4,5]执行concat操作：【1，2，3，4，5】。而Add是代数相加
    #---------------------------------------------------#
    #   第三个特征层
...
def get_train_model(model_body, input_shape, num_classes, anchors, anchors_mask):
    ...
    model_loss  = Lambda(#Lambda函数：见csdn"王毛收藏夹"
        yolo_loss,
        output_shape    = (1, ),
        name            = 'yolo_loss',
        arguments       = {
